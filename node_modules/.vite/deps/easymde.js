...
//#endregion
//#region node_modules/easymde/src/js/codemirror/tablist.js
var require_tablist = /* @__PURE__ */ __commonJSMin((() => {
	var CodeMirror = require_codemirror();
	CodeMirror.commands.tabAndIndentMarkdownList = function(cm) {
		var pos = cm.listSelections()[0].head;
		if (cm.getStateAfter(pos.line).list !== false) {
...
//#endregion
//#region node_modules/codemirror/addon/display/fullscreen.js
var require_fullscreen = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	(function(mod) {
		if (typeof exports == "object" && typeof module == "object") mod(require_codemirror());
		else if (typeof define == "function" && define.amd) define(["../../lib/codemirror"], mod);
		else mod(CodeMirror);
...
//#endregion
//#region node_modules/codemirror/addon/mode/overlay.js
...
//#endregion
//#region node_modules/codemirror/addon/display/placeholder.js
...
//#endregion
//#region node_modules/codemirror/addon/selection/mark-selection.js
var require_mark_selection = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	(function(mod) {
		if (typeof exports == "object" && typeof module == "object") mod(require_codemirror());
		else if (typeof define == "function" && define.amd) define(["../../lib/codemirror"], mod);
		else mod(CodeMirror);
		...
		function update(cm) {
			...
			if (cmp(from, coverStart.from) < 0) if (coverStart.to.line - from.line < CHUNK_SIZE) {
				array.shift().clear();
...
//#endregion
//#region node_modules/codemirror/addon/search/searchcursor.js
...
//#endregion
//#region node_modules/codemirror/mode/gfm/gfm.js
var require_gfm = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	(function(mod) {
		if (typeof exports == "object" && typeof module == "object") mod(require_codemirror(), require_markdown(), require_overlay());
		else if (typeof define == "function" && define.amd) define([
			"../../lib/codemirror",
			"../markdown/markdown",
			"../../addon/mode/overlay"
				...
				token: function(stream, state) {
					...
					if (stream.peek() === "`") {
						...
						if (!state.code) {
							...
							state.code = true;
						} else if (difference === codeDepth) state.code = false;
						return null;
					} else if (state.code) {
						stream.next();
						return null;
...
//#endregion
//#region node_modules/typo-js/typo.js
var require_typo = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	var Typo;
	(function() {
		"use strict";
		/**
		...
		*/
		Typo = function(dictionary, affData, wordsData, settings) {
			...
			if (dictionary) {
				...
				if (affData && wordsData) setup();
				else {
					if (typeof window !== "undefined") {
						if (settings.dictionaryPath) path = settings.dictionaryPath;
						else path = "typo/dictionaries";
						if (window.chrome && window.chrome.runtime && window.chrome.runtime.getURL) path = window.chrome.runtime.getURL(path);
						else if (window.browser && window.browser.runtime && window.browser.runtime.getURL) path = window.browser.runtime.getURL(path);
					} else if (typeof __dirname !== "undefined") path = __dirname + "/dictionaries";
			...
			function readDataFile(url, setFunc) {
				...
				if (settings === null || settings === void 0 ? void 0 : settings.asyncLoad) response.then(function(data) {
					setFunc(data);
				});
			...
			function setup() {
				...
				for (i = 0, _len = self.compoundRules.length; i < _len; i++) {
					...
					for (j = 0, _jlen = rule.length; j < _jlen; j++) self.compoundRuleCodes[rule[j]] = [];
				}
				if ("ONLYINCOMPOUND" in self.flags) self.compoundRuleCodes[self.flags.ONLYINCOMPOUND] = [];
				self.dictionaryTable = self._parseDIC(wordsData);
				for (i in self.compoundRuleCodes) if (self.compoundRuleCodes[i].length === 0) delete self.compoundRuleCodes[i];
				for (i = 0, _len = self.compoundRules.length; i < _len; i++) {
					var ruleText = self.compoundRules[i];
					var expressionText = "";
					for (j = 0, _jlen = ruleText.length; j < _jlen; j++) {
						var character = ruleText[j];
						if (character in self.compoundRuleCodes) expressionText += "(" + self.compoundRuleCodes[character].join("|") + ")";
						else expressionText += character;
					}
					self.compoundRules[i] = new RegExp("^" + expressionText + "$", "i");
				}
				self.loaded = true;
				if ((settings === null || settings === void 0 ? void 0 : settings.asyncLoad) && (settings === null || settings === void 0 ? void 0 : settings.loadedCallback)) settings.loadedCallback(self);
			}
			return this;
		};
		Typo.prototype = {
			...
			*/
			load: function(obj) {
				for (var i in obj) if (obj.hasOwnProperty(i)) this[i] = obj[i];
				return this;
			},
			/**
			...
			*/
			_readFile: function(path, charset, async) {
				...
				if (typeof XMLHttpRequest !== "undefined") {
					...
					(_a = req_1.overrideMimeType) === null || _a === void 0 || _a.call(req_1, "text/plain; charset=" + charset);
					if (!!async) {
						var promise = new Promise(function(resolve, reject) {
							req_1.onload = function() {
								if (req_1.status === 200) resolve(req_1.responseText);
								else reject(req_1.statusText);
							};
					...
					try {
						if (fs.existsSync(path)) return fs.readFileSync(path, charset);
						else console.log("Path " + path + " does not exist.");
					} catch (e) {
						console.log(e);
					}
					return "";
				...
				for (i = 0, _len = lines.length; i < _len; i++) {
					...
					if (ruleType === "PFX" || ruleType === "SFX") {
						...
						for (j = i + 1, _jlen = i + 1 + numEntries; j < _jlen; j++) {
							...
							if (regexToMatch !== ".") if (ruleType === "SFX") entry.match = new RegExp(regexToMatch + "$");
						...
						lineParts = line.split(/\s+/);
						if (lineParts.length === 3) this.replacementTable.push([lineParts[1], lineParts[2]]);
			...
			*/
			_parseDIC: function(data) {
				...
				for (var i = 1, _len = lines.length; i < _len; i++) {
					var line = lines[i];
					if (!line) continue;
					var parts = line.replace(/\s.*$/, "").split("/", 2);
					var word = parts[0];
					if (parts.length > 1) {
						var ruleCodesArray = this.parseRuleCodes(parts[1]);
						if (!("NEEDAFFIX" in this.flags) || ruleCodesArray.indexOf(this.flags.NEEDAFFIX) === -1) addWord(word, ruleCodesArray);
						for (var j = 0, _jlen = ruleCodesArray.length; j < _jlen; j++) {
							...
							if (rule) {
								...
								for (var ii = 0, _iilen = newWords.length; ii < _iilen; ii++) {
									...
									addWord(newWord, []);
									if (rule.combineable) for (var k = j + 1; k < _jlen; k++) {
			...
			*/
			...
			parseRuleCodes: function(textCodes) {
				if (!textCodes) return [];
					...
					var flags = [];
					for (var i = 0, _len = textCodes.length; i < _len; i += 2) flags.push(textCodes.substr(i, 2));
					return flags;
				} else if (this.flags.FLAG === "num") return textCodes.split(",");
				else if (this.flags.FLAG === "UTF-8") return Array.from(textCodes);
			...
			_applyRule: function(word, rule) {
				...
				for (var i = 0, _len = entries.length; i < _len; i++) {
					...
					if (!entry.match || word.match(entry.match)) {
						...
						if ("continuationClasses" in entry) for (var j = 0, _jlen = entry.continuationClasses.length; j < _jlen; j++) {
							var continuationRule = this.rules[entry.continuationClasses[j]];
							if (continuationRule) newWords = newWords.concat(this._applyRule(newWord, continuationRule));
						}
					}
				}
				return newWords;
			...
			checkExact: function(word) {
				...
				if (typeof ruleCodes === "undefined") {
					if ("COMPOUNDMIN" in this.flags && word.length >= this.flags.COMPOUNDMIN) {
						for (i = 0, _len = this.compoundRules.length; i < _len; i++) if (word.match(this.compoundRules[i])) return true;
					}
				} else if (ruleCodes === null) return true;
				else if (typeof ruleCodes === "object") {
					for (i = 0, _len = ruleCodes.length; i < _len; i++) if (!this.hasFlag(word, "ONLYINCOMPOUND", ruleCodes[i])) return true;
				}
				return false;
			},
			/**
			...
			*/
			hasFlag: function(word, flag, wordFlags) {
				...
				if (flag in this.flags) {
					if (typeof wordFlags === "undefined") wordFlags = Array.prototype.concat.apply([], this.dictionaryTable.get(word));
			...
			*/
			...
			suggest: function(word, limit) {
				...
				if (this.memoized.hasOwnProperty(word)) {
					...
					if (limit <= memoizedLimit || this.memoized[word]["suggestions"].length < memoizedLimit) return this.memoized[word]["suggestions"].slice(0, limit);
				}
				...
				*/
				function edits1(words, known_only) {
					var rv = {}, i, j, _len, _edit;
					var alphabetLength = self.alphabet.length;
					for (var word_1 in words) for (i = 0, _len = word_1.length + 1; i < _len; i++) {
						var s = [word_1.substring(0, i), word_1.substring(i)];
						if (s[1]) {
							_edit = s[0] + s[1].substring(1);
							if (!known_only || self.check(_edit)) if (!(_edit in rv)) rv[_edit] = 1;
							else rv[_edit] += 1;
						}
						...
						if (s[1]) for (j = 0; j < alphabetLength; j++) {
				...
				function correct(word) {
					...
					var weighted_corrections = edits1(ed1, true);
					for (var ed1word in ed1) {
						if (!self.check(ed1word)) continue;
						if (ed1word in weighted_corrections) weighted_corrections[ed1word] += ed1[ed1word];
						else weighted_corrections[ed1word] = ed1[ed1word];
					}
					var i;
					var sorted_corrections = [];
					for (i in weighted_corrections) if (weighted_corrections.hasOwnProperty(i)) {
						if (self.hasFlag(i, "PRIORITYSUGGEST")) weighted_corrections[i] += 1e3;
					...
					for (i = 0; i < Math.min(working_limit, sorted_corrections.length); i++) {
						...
						if (!self.hasFlag(sorted_corrections[i][0], "NOSUGGEST") && rv.indexOf(sorted_corrections[i][0]) === -1) rv.push(sorted_corrections[i][0]);
						else working_limit++;
					}
					return rv;
				}
	...
	if (typeof module !== "undefined") module.exports = Typo;
}));
//#endregion
//#region node_modules/codemirror-spell-checker/src/js/spell-checker.js
var require_spell_checker = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	...
	function CodeMirrorSpellChecker(options) {
		...
		if (!String.prototype.includes) String.prototype.includes = function() {
			"use strict";
		...
		options.codeMirrorInstance.defineMode("spell-checker", function(config) {
			if (!CodeMirrorSpellChecker.aff_loading) {
				...
				xhr_aff.onload = function() {
					if (xhr_aff.readyState === 4 && xhr_aff.status === 200) {
						...
						if (CodeMirrorSpellChecker.num_loaded == 2) CodeMirrorSpellChecker.typo = new Typo("en_US", CodeMirrorSpellChecker.aff_data, CodeMirrorSpellChecker.dic_data, { platform: "any" });
					}
				};
				xhr_aff.send(null);
			}
			...
			var overlay = { token: function(stream) {
				...
				if (CodeMirrorSpellChecker.typo && !CodeMirrorSpellChecker.typo.check(word)) return "spell-error";
				return null;
			} };
...
//#endregion
//#region node_modules/marked/lib/marked.cjs
/**
...
*/
var require_marked = /* @__PURE__ */ __commonJSMin(((exports) => {
			...
			if (cap) {
				...
				for (i = 0; i < l; i++) {
					...
					if (!list.loose) {
						...
						list.loose = spacers.length > 0 && spacers.some(function(t) {
							return /\n.*\n/.test(t.raw);
						});
					}
				}
	...
	*/
	marked.use = function() {
		...
		args.forEach(function(pack) {
			...
			if (pack.hooks) (function() {
				var hooks = marked.defaults.hooks || new Hooks();
				var _loop3 = function _loop3(prop) {
					var prevHook = hooks[prop];
					if (Hooks.passThroughHooks.has(prop)) hooks[prop] = function(arg) {
						if (marked.defaults.async) return Promise.resolve(pack.hooks[prop].call(hooks, arg)).then(function(ret) {
							return prevHook.call(hooks, ret);
						});
						var ret = pack.hooks[prop].call(hooks, arg);
						return prevHook.call(hooks, ret);
					};
					else hooks[prop] = function() {
				...
				for (var prop in pack.hooks) _loop3(prop);
				opts.hooks = hooks;
			})();
			if (pack.walkTokens) {
				...
				opts.walkTokens = function(token) {
					...
					if (_walkTokens) values = values.concat(_walkTokens.call(this, token));
					return values;
				};
			}
		...
		var _loop4 = function _loop4() {
			...
			switch (token.type) {
				...
				case "list":
					...
					break;
				default: if (marked.defaults.extensions && marked.defaults.extensions.childTokens && marked.defaults.extensions.childTokens[token.type]) marked.defaults.extensions.childTokens[token.type].forEach(function(childTokens) {
					values = values.concat(marked.walkTokens(token[childTokens], callback));
				});
				else if (token.tokens) values = values.concat(marked.walkTokens(token.tokens, callback));
			}
		};
		for (var _iterator = _createForOfIteratorHelperLoose(tokens), _step; !(_step = _iterator()).done;) _loop4();
		return values;
	};
	/**
	* Parse Inline
...
//#endregion
//#region node_modules/easymde/src/js/easymde.js
var require_easymde = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	var CodeMirror = require_codemirror();
	...
	var anchorToExternalRegex = /* @__PURE__ */ new RegExp(/(<a.*?https?:\/\/.*?[^a]>)+?/g);
	var bindings = {
	...
	var getBindingName = function(f) {
		for (var key in bindings) if (bindings[key] === f) return key;
		return null;
	};
	...
	*/
	function createToolbarDropdown(options, enableTooltips, shortcuts, parent) {
		...
		for (var childrenIndex = 0; childrenIndex < options.children.length; childrenIndex++) {
			...
			var childElement;
			if (typeof child === "string" && child in toolbarBuiltInButtons) childElement = createToolbarButton(toolbarBuiltInButtons[child], true, enableTooltips, shortcuts, "button", parent);
			else childElement = createToolbarButton(child, true, enableTooltips, shortcuts, "button", parent);
	...
	*/
	function createToolbarButton(options, enableActions, enableTooltips, shortcuts, markup, parent) {
		...
		if (options.attributes) {
			for (var attribute in options.attributes) if (Object.prototype.hasOwnProperty.call(options.attributes, attribute)) el.setAttribute(attribute, options.attributes[attribute]);
		}
		el.className = (parent.options.toolbarButtonClassPrefix ? parent.options.toolbarButtonClassPrefix + "-" : "") + options.name;
		el.setAttribute("type", markup);
		enableTooltips = enableTooltips == void 0 ? true : enableTooltips;
		if (options.text) el.innerText = options.text;
		if (options.name && options.name in shortcuts) bindings[options.name] = options.action;
		...
		if (options.title) el.setAttribute("aria-label", options.title);
		if (options.noDisable) el.classList.add("no-disable");
		if (options.noMobile) el.classList.add("no-mobile");
		var classNameParts = [];
		if (typeof options.className !== "undefined") classNameParts = options.className.split(" ");
		var iconClasses = [];
		for (var classNameIndex = 0; classNameIndex < classNameParts.length; classNameIndex++) {
			var classNamePart = classNameParts[classNameIndex];
			if (classNamePart.match(/^fa([srlb]|(-[\w-]*)|$)/)) iconClasses.push(classNamePart);
		...
		if (typeof options.icon !== "undefined") el.innerHTML = options.icon;
		if (options.action && enableActions) {
			if (typeof options.action === "function") el.onclick = function(e) {
				e.preventDefault();
				options.action(parent);
			};
			else if (typeof options.action === "string") el.onclick = function(e) {
				e.preventDefault();
	...
	function createTooltip(title, action, shortcuts) {
		...
		if (action) {
			...
			if (shortcuts[actionName]) tooltip += " (" + fixShortcut(shortcuts[actionName]) + ")";
		}
		return tooltip;
	}
	/**
	...
	function getState(cm, pos) {
		...
		for (var i = 0; i < types.length; i++) {
			...
			if (data === "strong") ret.bold = true;
				...
				else if (/^\s*- \[[ xX]]\s/.test(text)) ret["check-list"] = true;
			...
			else if (data === "link" && !ret.image) ret.link = true;
			else if (data === "image") ret.image = true;
	...
	*/
	function toggleFullScreen(editor) {
		...
		} else document.body.style.overflow = saved_overflow;
		var wrapper = cm.getWrapperElement();
		var sidebyside = wrapper.nextSibling;
		if (sidebyside.classList.contains("editor-preview-active-side")) if (editor.options.sideBySideFullscreen === false) {
			var easyMDEContainer = wrapper.parentNode;
			if (cm.getOption("fullScreen")) easyMDEContainer.classList.remove("sided--no-fullscreen");
			else easyMDEContainer.classList.add("sided--no-fullscreen");
		} else toggleSideBySide(editor);
		if (editor.options.onToggleFullScreen) editor.options.onToggleFullScreen(cm.getOption("fullScreen") || false);
		if (typeof editor.options.maxHeight !== "undefined") if (cm.getOption("fullScreen")) {
		...
		if (editor.toolbarElements && editor.toolbarElements.fullscreen) editor.toolbarElements.fullscreen.classList.toggle("active");
	}
	/**
	...
	*/
	function toggleCodeBlock(editor) {
		...
		function code_type(cm, line_num, line, firstTok, lastTok) {
			...
			else if (types.indexOf("comment") === -1) return false;
			else if (token_state(firstTok).fencedChars || token_state(lastTok).fencedChars || fencing_line(line)) return "fenced";
			else return "single";
		}
		...
		if (is_code === "single") {
			...
			cur_start.ch--;
			if (cur_start !== cur_end) cur_end.ch--;
			cm.setSelection(cur_start, cur_end);
			cm.focus();
		} else if (is_code === "fenced") if (cur_start.line !== cur_end.line || cur_start.ch !== cur_end.ch) {
			for (block_start = cur_start.line; block_start >= 0; block_start--) {
				line = cm.getLineHandle(block_start);
				if (fencing_line(line)) break;
			}
			var fence_chars = token_state(cm.getTokenAt({
				line: block_start,
				ch: 1
			})).fencedChars;
			...
			if (fencing_line(cm.getLineHandle(cur_start.line))) if (code_type(cm, cur_start.line + 1) === "fenced") {
			...
			if (block_start === void 0) for (block_start = search_from; block_start >= 0; block_start--) {
				line = cm.getLineHandle(block_start);
				if (fencing_line(line)) break;
			}
			if (block_end === void 0) {
			...
			if (cur_start.line !== cur_end.line || cur_start.ch !== cur_end.ch) {
				...
				for (block_start = cur_start.line; block_start >= 0; block_start--) {
					line = cm.getLineHandle(block_start);
					if (line.text.match(/^\s*$/)) continue;
					else if (code_type(cm, block_start, line) !== "indented") {
						block_start += 1;
						break;
					}
				}
				...
				for (block_end = cur_start.line; block_end < lineCount; block_end++) {
					line = cm.getLineHandle(block_end);
					if (line.text.match(/^\s*$/)) continue;
					else if (code_type(cm, block_end, line) !== "indented") {
						block_end -= 1;
						break;
					}
				}
			...
			if (next_line_last_tok && token_state(next_line_last_tok).indentedCode) cm.replaceRange("\n", {
				line: block_end + 1,
				ch: 0
			});
			...
			if (no_sel_and_starting_of_line || sel_multi) insertFencingAtSelection(cm, cur_start, cur_end, fenceCharsToInsert);
			else _replaceSelection(cm, false, ["`", "`"]);
		}
	}
	/**
	...
	*/
	function toggleUnorderedList(editor) {
		...
		if ([
			...
			"+",
			"*"
		].includes(editor.options.unorderedListStyle)) listStyle = editor.options.unorderedListStyle;
	...
	*/
	function afterImageUploaded(editor, url) {
		...
		if ([
			...
			"avif",
			"webp"
		].includes(ext)) _replaceSelection(cm, stat.image, options.insertTexts.uploadedImage, url);
		else {
	...
	*/
	function toggleSideBySide(editor) {
		...
		if (preview.classList.contains("editor-preview-active-side")) {
			if (editor.options.sideBySideFullscreen === false) easyMDEContainer.classList.remove("sided--no-fullscreen");
			...
			setTimeout(function() {
				if (!cm.getOption("fullScreen")) if (editor.options.sideBySideFullscreen === false) easyMDEContainer.classList.add("sided--no-fullscreen");
				else toggleFullScreen(editor);
		...
		if (!cm.sideBySideRenderingFunction) cm.sideBySideRenderingFunction = sideBySideRenderingFunction;
		if (useSideBySideListener) {
			var newValue = editor.options.previewRender(editor.value(), preview);
			if (newValue != null) preview.innerHTML = newValue;
			cm.on("update", cm.sideBySideRenderingFunction);
		} else cm.off("update", cm.sideBySideRenderingFunction);
		cm.refresh();
	}
	/**
	* Preview action.
	...
	*/
	function togglePreview(editor) {
		...
		if (cm.getWrapperElement().nextSibling.classList.contains("editor-preview-active-side")) toggleSideBySide(editor);
		if (!preview || !preview.classList.contains("editor-preview-full")) {
			preview = document.createElement("div");
			preview.className = "editor-preview-full";
			if (editor.options.previewClass) {
				if (Array.isArray(editor.options.previewClass)) for (var i = 0; i < editor.options.previewClass.length; i++) preview.classList.add(editor.options.previewClass[i]);
				else if (typeof editor.options.previewClass === "string") preview.classList.add(editor.options.previewClass);
			}
		...
		if (preview_result !== null) preview.innerHTML = preview_result;
	}
	...
	function _toggleHeading(cm, direction, size) {
		...
		for (var i = startPoint.line; i <= endPoint.line; i++) (function(i) {
			var text = cm.getLine(i);
			var currHeadingLevel = text.search(/[^#]/);
			if (direction !== void 0) if (currHeadingLevel <= 0) if (direction == "bigger") text = "###### " + text;
			else text = "# " + text;
			...
			else if (direction == "bigger") text = text.substr(1);
			else text = "#" + text;
			else if (currHeadingLevel <= 0) text = "#".repeat(size) + " " + text;
			else if (currHeadingLevel == size) text = text.substr(currHeadingLevel + 1);
			else text = "#".repeat(size) + " " + text.substr(currHeadingLevel + 1);
	...
	function _toggleLine(cm, name, liststyle) {
		...
		var _toggle = function(name, text, untoggleOnly) {
			...
			if (arr !== null) {
				if (_checkChar(name, arr[2])) char = "";
				text = arr[1] + char + arr[3] + text.replace(whitespacesRegexp, "").replace(repl[name], "$1");
			} else if (untoggleOnly == false) text = char + " " + text;
			return text;
		};
		var line = 1;
		var listTypes = [
	...
	*/
	function _toggleLink(editor, type, startEnd, url) {
		...
		if (type == "link") start = start.replace(/(.*)[^!]\[/, "$1");
		else if (type == "image") start = start.replace(/(.*)!\[$/, "$1");
		end = end.replace(/]\(.*?\)/, "");
		cm.replaceRange(start + end, {
			line: startPoint.line,
			ch: 0
	...
	*/
	function humanFileSize(bytes, units) {
		if (Math.abs(bytes) < 1024) return "" + bytes + units[0];
		var u = 0;
		do {
			bytes /= 1024;
			++u;
	...
	function _mergeProperties(target, source) {
		for (var property in source) if (Object.prototype.hasOwnProperty.call(source, property)) if (source[property] instanceof Array) target[property] = source[property].concat(target[property] instanceof Array ? target[property] : []);
		else if (source[property] !== null && typeof source[property] === "object" && source[property].constructor === Object) target[property] = _mergeProperties(target[property] || {}, source[property]);
		else target[property] = source[property];
		return target;
	}
	function extend(target) {
		for (var i = 1; i < arguments.length; i++) target = _mergeProperties(target, arguments[i]);
		return target;
	}
	function wordCount(data) {
		var m = data.match(/[a-zA-Z0-9_\u00A0-\u02AF\u0392-\u03c9\u0410-\u04F9]+|[\u4E00-\u9FFF\u3400-\u4dbf\uf900-\ufaff\u3040-\u309f\uac00-\ud7af]+/g);
		var count = 0;
		if (m === null) return count;
		for (var i = 0; i < m.length; i++) if (m[i].charCodeAt(0) >= 19968) count += m[i].length;
		else count += 1;
		return count;
	}
	var iconClassMap = {
	...
	*/
	function EasyMDE(options) {
		...
		if (options.autoDownloadFontAwesome === false) autoDownloadFA = false;
		if (options.autoDownloadFontAwesome !== true) {
			var styleSheets = document.styleSheets;
			for (var i = 0; i < styleSheets.length; i++) {
				if (!styleSheets[i].href) continue;
				if (styleSheets[i].href.indexOf("//maxcdn.bootstrapcdn.com/font-awesome/") > -1) autoDownloadFA = false;
			}
		}
		if (autoDownloadFA) {
		...
		if (options.toolbar === void 0) {
			options.toolbar = [];
			for (var key in toolbarBuiltInButtons) if (Object.prototype.hasOwnProperty.call(toolbarBuiltInButtons, key)) {
		...
		if (!Object.prototype.hasOwnProperty.call(options, "previewClass")) options.previewClass = "editor-preview";
		if (!Object.prototype.hasOwnProperty.call(options, "status")) {
			...
			if (options.uploadImage) options.status.unshift("upload-image");
		}
		if (!options.previewRender) options.previewRender = function(plainText) {
			return this.parent.markdown(plainText);
		};
		options.parsingConfig = extend({ highlightFormatting: true }, options.parsingConfig || {});
		options.insertTexts = extend({}, insertTexts, options.insertTexts || {});
		options.promptTexts = extend({}, promptTexts, options.promptTexts || {});
		options.blockStyles = extend({}, blockStyles, options.blockStyles || {});
		if (options.autosave != void 0) options.autosave.timeFormat = extend({}, timeFormat, options.autosave.timeFormat || {});
		...
		if (typeof options.maxHeight !== "undefined") options.minHeight = options.maxHeight;
		else options.minHeight = options.minHeight || "300px";
		...
		if (options.autosave != void 0 && options.autosave.unique_id != void 0 && options.autosave.unique_id != "") options.autosave.uniqueId = options.autosave.unique_id;
		if (options.overlayMode && options.overlayMode.combine === void 0) options.overlayMode.combine = true;
		this.options = options;
		this.render();
		if (options.initialValue && (!this.options.autosave || this.options.autosave.foundSavedValue !== true)) this.value(options.initialValue);
		if (options.uploadImage) {
			...
			this.codemirror.on("drop", function(cm, event) {
				event.stopPropagation();
				event.preventDefault();
				if (options.imageUploadFunction) self.uploadImagesUsingCustomFunction(options.imageUploadFunction, event.dataTransfer.files);
				else self.uploadImages(event.dataTransfer.files);
			});
			this.codemirror.on("paste", function(cm, event) {
				if (options.imageUploadFunction) self.uploadImagesUsingCustomFunction(options.imageUploadFunction, event.clipboardData.files);
				else self.uploadImages(event.clipboardData.files);
			});
		}
	}
	/**
	...
	*/
	EasyMDE.prototype.updateStatusBar = function(itemName, content) {
		if (!this.gui.statusbar) return;
		var matchingClasses = this.gui.statusbar.getElementsByClassName(itemName);
		if (matchingClasses.length === 1) this.gui.statusbar.getElementsByClassName(itemName)[0].textContent = content;
		else if (matchingClasses.length === 0) console.log("EasyMDE: status bar item " + itemName + " was not found.");
		else console.log("EasyMDE: Several status bar items named " + itemName + " was found.");
	};
	/**
	...
	*/
	EasyMDE.prototype.markdown = function(text) {
		if (marked) {
			var markedOptions;
			if (this.options && this.options.renderingConfig && this.options.renderingConfig.markedOptions) markedOptions = this.options.renderingConfig.markedOptions;
			else markedOptions = {};
			if (this.options && this.options.renderingConfig && this.options.renderingConfig.singleLineBreaks === false) markedOptions.breaks = false;
			else markedOptions.breaks = true;
			if (this.options && this.options.renderingConfig && this.options.renderingConfig.codeSyntaxHighlighting === true) {
				var hljs = this.options.renderingConfig.hljs || window.hljs;
				if (hljs) markedOptions.highlight = function(code, language) {
			...
			if (this.options.renderingConfig && typeof this.options.renderingConfig.sanitizerFunction === "function") htmlText = this.options.renderingConfig.sanitizerFunction.call(this, htmlText);
	...
	*/
	EasyMDE.prototype.render = function(el) {
		if (!el) el = this.element || document.getElementsByTagName("textarea")[0];
		if (this._rendered && this._rendered === el) return;
		this.element = el;
		var options = this.options;
		var self = this;
		var keyMaps = {};
		for (var key in options.shortcuts) if (options.shortcuts[key] !== null && bindings[key] !== null) (function(key) {
			keyMaps[fixShortcut(options.shortcuts[key])] = function() {
				var action = bindings[key];
				if (typeof action === "function") action(self);
				else if (typeof action === "string") window.open(action, "_blank");
			};
		})(key);
		...
		if (options.spellChecker !== false) {
			...
			backdrop.name = "gfm";
			backdrop.gitHubSpice = false;
			if (typeof options.spellChecker === "function") options.spellChecker({ codeMirrorInstance: CodeMirror });
			else CodeMirrorSpellChecker({ codeMirrorInstance: CodeMirror });
		}
		function configureMouse(cm, repeat, event) {
			return { addNew: false };
		}
		...
		if (typeof options.maxHeight !== "undefined") this.codemirror.getScrollerElement().style.height = options.maxHeight;
		...
		if (options.toolbar !== false) this.gui.toolbar = this.createToolbar();
		...
		function calcHeight(naturalWidth, naturalHeight) {
			var height;
			if (naturalWidth < window.getComputedStyle(document.querySelector(".CodeMirror-sizer")).width.replace("px", "")) height = naturalHeight + "px";
			else height = naturalHeight / naturalWidth * 100 + "%";
			return height;
		}
		var _vm = this;
				...
				if (!parentEl.hasAttribute("data-img-src")) {
					...
					if (srcAttr && srcAttr.length >= 2) {
						...
						if (options.imagesPreviewHandler) {
							...
							if (typeof newSrc === "string") keySrc = newSrc;
						}
						if (!window.EMDEimagesCache[keySrc]) {
							...
							img.src = keySrc;
						} else assignImageBlockAttributes(parentEl, window.EMDEimagesCache[keySrc]);
					}
				}
			});
		}
		...
		if (options.autofocus === true || el.autofocus) this.codemirror.focus();
	...
	EasyMDE.prototype.autosave = function() {
		if (isLocalStorageAvailable()) {
			...
			if (this.options.autosave.binded !== true) {
				if (easyMDE.element.form != null && easyMDE.element.form != void 0) easyMDE.element.form.addEventListener("submit", function() {
			...
			if (value !== "") localStorage.setItem("smde_" + this.options.autosave.uniqueId, value);
			else localStorage.removeItem("smde_" + this.options.autosave.uniqueId);
			...
			if (el != null && el != void 0 && el != "") {
				...
				el.innerHTML = (this.options.autosave.text == void 0 ? "Autosaved: " : this.options.autosave.text) + dd;
			}
	...
	*/
	EasyMDE.prototype.openBrowseFileWindow = function(onSuccess, onError) {
		...
		function onChange(event) {
			if (self.options.imageUploadFunction) self.uploadImagesUsingCustomFunction(self.options.imageUploadFunction, event.target.files);
			else self.uploadImages(event.target.files, onSuccess, onError);
	...
	*/
	EasyMDE.prototype.uploadImage = function(file, onSuccess, onError) {
		...
		if (self.options.imageCSRFToken && !self.options.imageCSRFHeader) formData.append(self.options.imageCSRFName, self.options.imageCSRFToken);
		...
		if (self.options.imageCSRFToken && self.options.imageCSRFHeader) request.setRequestHeader(self.options.imageCSRFName, self.options.imageCSRFToken);
		request.onload = function() {
			...
			if (this.status === 200 && response && !response.error && response.data && response.data.filePath) onSuccess((self.options.imagePathAbsolute ? "" : window.location.origin + "/") + response.data.filePath);
			else if (response.error && response.error in self.options.errorMessages) onErrorSup(fillErrorMessage(self.options.errorMessages[response.error]));
			else if (response.error) onErrorSup(fillErrorMessage(response.error));
			else {
	...
	*/
	...
	EasyMDE.prototype.setPreviewMaxHeight = function() {
		...
		var previewMaxHeight = (parseInt(this.options.maxHeight) + paddingTop * 2 + borderTopWidth * 2).toString() + "px";
	...
	EasyMDE.prototype.createSideBySide = function() {
		...
		if (!preview || !preview.classList.contains("editor-preview-side")) {
			...
			if (this.options.previewClass) {
				if (Array.isArray(this.options.previewClass)) for (var i = 0; i < this.options.previewClass.length; i++) preview.classList.add(this.options.previewClass[i]);
				else if (typeof this.options.previewClass === "string") preview.classList.add(this.options.previewClass);
			}
			wrapper.parentNode.insertBefore(preview, wrapper.nextSibling);
		}
		if (typeof this.options.maxHeight !== "undefined") this.setPreviewMaxHeight();
	...
	EasyMDE.prototype.createToolbar = function(items) {
		...
		var i;
		for (i = 0; i < items.length; i++) if (toolbarBuiltInButtons[items[i]] != void 0) items[i] = toolbarBuiltInButtons[items[i]];
		...
		for (i = 0; i < items.length; i++) {
			...
			if (items[i] === "|") {
				...
				for (var x = i + 1; x < items.length; x++) if (items[x] !== "|" && (!self.options.hideIcons || self.options.hideIcons.indexOf(items[x].name) == -1)) nonSeparatorIconsFollow = true;
				if (!nonSeparatorIconsFollow) continue;
			}
			(function(item) {
				var el;
				if (item === "|") el = createSep();
				else if (item.children) el = createToolbarDropdown(item, self.options.toolbarTips, self.options.shortcuts, self);
				else el = createToolbarButton(item, true, self.options.toolbarTips, self.options.shortcuts, "button", self);
		...
		cm.on("cursorActivity", function() {
			var stat = getState(cm);
			for (var key in toolbarData) (function(key) {
				var el = toolbarData[key];
				if (stat[key]) el.classList.add("active");
				else if (key != "fullscreen" && key != "side-by-side") el.classList.remove("active");
			})(key);
		});
	...
	EasyMDE.prototype.createStatusbar = function(status) {
		...
		for (i = 0; i < status.length; i++) {
			...
			onActivity = void 0;
			defaultValue = void 0;
			if (typeof status[i] === "object") items.push({
				...
				} else if (name === "autosave") defaultValue = function(el) {
					if (options.autosave != void 0 && options.autosave.enabled === true) el.setAttribute("id", "autosaved");
				};
				else if (name === "upload-image") defaultValue = function(el) {
		...
		for (i = 0; i < items.length; i++) {
			...
			if (typeof item.defaultValue === "function") item.defaultValue(el);
			if (typeof item.onUpdate === "function") this.codemirror.on("update", function(el, item) {
				return function() {
					item.onUpdate(el);
				};
			}(el, item));
			if (typeof item.onActivity === "function") this.codemirror.on("cursorActivity", function(el, item) {
				return function() {
					item.onActivity(el);
				};
			}(el, item));
	...
	*/
	EasyMDE.prototype.value = function(val) {
		var cm = this.codemirror;
		if (val === void 0) return cm.getValue();
		else {
	...
	*/
	...
	EasyMDE.prototype.toTextArea = function() {
		...
		var easyMDEContainer = wrapper.parentNode;
		if (easyMDEContainer) {
			if (this.gui.toolbar) easyMDEContainer.removeChild(this.gui.toolbar);
			if (this.gui.statusbar) easyMDEContainer.removeChild(this.gui.statusbar);
			if (this.gui.sideBySide) easyMDEContainer.removeChild(this.gui.sideBySide);
		}
